...
const projects = [
  { title: "Professional Portfolio", description: "A responsive developer portfolio built with React, Tailwind CSS and Framer Motion.", technologies: ["React", "Tailwind CSS", "Framer Motion"], icon: MdOutlineWeb, status: "In progress", github: "https://github.com/parash0725/awesome-portfolio", demo: "#home", featured: true },
  { title: "Restaurant Website", description: "A modern restaurant website with a digital menu, reservation section and mobile-friendly design.", technologies: ["React", "Tailwind CSS", "JavaScript"], icon: FaUtensils, status: "Planned", github: "", demo: "", featured: false },
...
function Projects() {
  return <section id="projects" className="bg-slate-900/50 px-6 py-24"><div className="mx-auto max-w-6xl">
    <motion.div initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true, amount: 0.2 }} transition={{ duration: 0.6 }} className="max-w-3xl">
      <p className="mb-3 font-semibold uppercase tracking-[0.25em] text-cyan-400">Selected work</p><h2 className="text-4xl font-bold sm:text-5xl">Projects I am building</h2><p className="mt-6 text-lg leading-8 text-slate-400">These projects represent my development roadmap. Completed projects will include live demonstrations, source code and clear documentation.</p>
    </motion.div>
    <div className="mt-14 grid gap-7 md:grid-cols-2 lg:grid-cols-3">{projects.map((project, index) => { const Icon = project.icon; return <motion.article key={project.title} initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true, amount: 0.15 }} transition={{ duration: 0.5, delay: index * 0.08 }} className={`group flex h-full flex-col rounded-2xl border bg-slate-950/70 p-7 transition hover:-translate-y-2 ${project.featured ? "border-cyan-400/40 hover:border-cyan-400" : "border-slate-800 hover:border-cyan-400/70"}`}>
      <div className="flex items-start justify-between"><div className="flex h-14 w-14 items-center justify-center rounded-xl bg-cyan-400/10 text-2xl text-cyan-400"><Icon /></div><span className={`rounded-full px-3 py-1 text-xs font-semibold ${project.status === "In progress" ? "bg-amber-400/10 text-amber-300" : "bg-slate-800 text-slate-400"}`}>{project.status}</span></div>
      <h3 className="mt-6 text-2xl font-bold transition group-hover:text-cyan-400">{project.title}</h3><p className="mt-4 flex-grow leading-7 text-slate-400">{project.description}</p>
      ...
      <div className="mt-7 flex items-center gap-5">{project.github ? <a href={project.github} target="_blank" rel="noreferrer" className="flex items-center gap-2 text-sm font-semibold text-slate-300 transition hover:text-cyan-400"><FaGithub />Source code</a> : <span className="flex items-center gap-2 text-sm text-slate-600"><FaGithub />Coming soon</span>}{project.demo ? <a href={project.demo} className="flex items-center gap-2 text-sm font-semibold text-slate-300 transition hover:text-cyan-400"><FaExternalLinkAlt />Live demo</a> : <span className="flex items-center gap-2 text-sm text-slate-600"><FaExternalLinkAlt />Not available</span>}</div>
    </motion.article>; })}</div>
  </div></section>;
}
